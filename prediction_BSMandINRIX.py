...
def main(argv):
	...
	if rank == 0:
		...
		conn = sqlite3.connect('graphs_dbBSM.sqlite')
		curs = conn.cursor()
		# Load date sets
		...
		for y in training_date_hours:
			...
			try:
				...
				pass
		logger.write("Training dates complete\n")
		logger.flush()
		# Initial screen calculation
		initial_screen = {}
		THRESH_MU = 0.2 # Note: threshold should be .2
	...
	vminrix = comm.bcast(vminrix, root=0)
	...
	logger.write("Calculating matched " + str(rank)+"\n")
	logger.flush()
	# validation and initial screen comparison
	matched = {}
	THRESH_E = 0.8 
	...
	conn = sqlite3.connect('counts_dbBSM.sqlite') # Assume this is database containing all data
	...
	conn2 = sqlite3.connect('counts_dbinrix.sqlite') # Assume this is database containing all data
	...
	for v in validation_date_hours:
		...
		for row1 in curs1:
			...
			curs2.execute('SELECT * FROM \'' + x + '\' WHERE id=? AND dir=? AND dt BETWEEN ? AND ? ORDER BY dt DESC LIMIT 1;', (boxid,direction,tstamp_v, tstamp_v))
			v_curr = curs2.fetchone()
			vm = vmbsm	
			matchdaytime = vdaytime
			bsmflag = True
			if v_curr == None:
				curs7.execute('SELECT * FROM \'' + x + '\' WHERE id=? AND dir=? AND dt BETWEEN ? AND ? ORDER BY dt DESC LIMIT 1;', (inrixboxid,direction,(tstamp_v- 14400), (tstamp_v- 14400)))
				v_curr = curs7.fetchone()
				vm = vminrix	
				matchdaytime = vdaytime - datetime.timedelta(hours = 4)
				bsmflag = False
			if v_curr == None:
				...
				bsmflag = True
			if v_curr == None:
				curs7.execute('SELECT * FROM \'' + x + '\' WHERE id=? AND dir=? AND dt BETWEEN ? AND ? ORDER BY dt DESC LIMIT 1;', (inrixboxid,direction,(tstamp_v- 14400)-3600, (tstamp_v- 14400)))
				v_curr = curs7.fetchone()		
				vm = vminrix	
				matchdaytime = vdaytime - datetime.timedelta(hours = 4)
				bsmflag = False						
			...
			if v_curr != None: 
				...
				# If there are matches, look for data on these training days
					...
					for tdaytime,ps in vm[matchdaytime]:
						...
						if bsmflag:
							...
							curs8.execute('SELECT * FROM \'' + y + '\' WHERE id=? AND dir=? AND dt=? LIMIT 1;', (inrixboxid,direction,tstamp_t))
							tr_fut = curs8.fetchone()
						...
						if tr_fut != None:
							...
							if bsmflag:
								...
								curs9.execute('SELECT * FROM \'' + y + '\' WHERE id=? AND dir=? AND dt BETWEEN ? AND ? ORDER BY dt DESC LIMIT 1;',(inrixboxid,direction,tstamp_t - 3600,tstamp_t))
								tr_curr = curs9.fetchone()
			...
			if final_prediction == None:
				print "fp None"
				continue
			# Make sure predictin is within bounds of bi
